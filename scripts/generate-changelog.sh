...
# Obtain tags from all branches.
PREVIOUS_RELEASE_TAG=$(git describe --tags $(git rev-list --tags --max-count=1))
...
set +e
HAS_CHANGES=$(diff -q ${CHANGELOG_TMP_FILE_NAME} ${CHANGELOG_FILE_NAME})
set -e
