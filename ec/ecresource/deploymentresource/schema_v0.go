...
func resourceStateUpgradeV0(_ context.Context, raw map[string]interface{}, m interface{}) (map[string]interface{}, error) {
	...
	for _, integrationsServer := range raw["integrations_server"].([]interface{}) {
		rawIntegrationsServer := integrationsServer.(map[string]interface{})
		delete(rawIntegrationsServer, "version")
	}
...
func resourceSchemaV0() *schema.Resource {
	return &schema.Resource{Schema: map[string]*schema.Schema{
		...
		"integrations_server": {
			Type:        schema.TypeList,
			Description: "Optional IntegrationsServer resource definition",
			Optional:    true,
			MaxItems:    1,
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
