...
func flattenApmResources(in []*models.ApmResourceInfo, name string) []interface{} {
	...
	for _, res := range in {
		...
		if cfg := flattenApmConfig(plan.Apm); len(cfg) > 0 {
			m["config"] = cfg
		}
...
func flattenApmConfig(cfg *models.ApmConfiguration) []interface{} {
	var m = make(map[string]interface{})
	if cfg == nil {
		return nil
	}
...
func flattenApmSystemConfig(cfg *models.ApmSystemSettings) map[string]interface{} {
	var m = make(map[string]interface{})
	if cfg == nil {
		return nil
	}
