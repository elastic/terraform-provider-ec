...
func Test_ensureMajorVersionStrategy(t *testing.T) {
	makePlan := func(groupBy string) *models.ElasticsearchPayload {
		return &models.ElasticsearchPayload{
			Plan: &models.ElasticsearchClusterPlan{
				Transient: &models.TransientElasticsearchPlanConfiguration{
					Strategy: &models.PlanStrategy{
						Rolling: &models.RollingStrategyConfig{GroupBy: groupBy},
					},
				},
			},
		}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			es := makePlan(tt.groupBy)
			ensureMajorVersionStrategy(es, tt.stateVersion, tt.planVersion)
			require.Equal(t, tt.wantGroupBy, es.Plan.Transient.Strategy.Rolling.GroupBy)
		})
	}
}
