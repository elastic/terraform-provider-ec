...
func readRequest(d *schema.ResourceData, client *api.API) (*extensions.GetExtensionOK, error) {
...
func createRequest(client *api.API, d *schema.ResourceData) (*extensions.CreateExtensionCreated, error) {
...
func updateRequest(client *api.API, d *schema.ResourceData) (*extensions.UpdateExtensionOK, error) {
	name := d.Get("name").(string)
	version := d.Get("version").(string)
	extensionsType := d.Get("extension_type").(string)
	description := d.Get("description").(string)
...
func deleteRequest(client *api.API, d *schema.ResourceData) (*extensions.DeleteExtensionOK, error) {
	res, err := client.V1API.Extensions.DeleteExtension(
...
func uploadRequest(client *api.API, d *schema.ResourceData) (*extensions.UploadExtensionOK, error) {

	reader, err := os.Open(d.Get("file_path").(string))
	if err != nil {
		return nil, multierror.NewPrefixed("failed open file", err)
	}

	res, err := client.V1API.Extensions.UploadExtension(
