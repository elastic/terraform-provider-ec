...
func flattenEnterpriseSearchResources(ctx context.Context, in []*models.EnterpriseSearchResourceInfo) (types.List, diag.Diagnostics) {
	...
	for _, res := range in {
		model := enterpriseSearchResourceInfoModelV0{
			Topology: types.ListNull(types.ObjectType{AttrTypes: enterpriseSearchTopologyAttrTypes()}),
		}
		...
		if res.Info != nil {
			...
			if !util.IsCurrentEssPlanEmpty(res) {
				...
				if plan.EnterpriseSearch != nil {
					model.Version = types.StringValue(plan.EnterpriseSearch.Version)
				}
	...
	target, diags := types.ListValueFrom(ctx, types.ObjectType{AttrTypes: enterpriseSearchResourceInfoAttrTypes()}, result)
	diagnostics.Append(diags...)
...
func flattenEnterpriseSearchTopology(ctx context.Context, plan *models.EnterpriseSearchPlan) (types.List, diag.Diagnostics) {
	...
	for _, topology := range plan.ClusterTopology {
		...
		if topology.NodeType != nil {
			if topology.NodeType.Appserver != nil {
				model.NodeTypeAppserver = types.BoolValue(*topology.NodeType.Appserver)
			}

			if topology.NodeType.Connector != nil {
				model.NodeTypeConnector = types.BoolValue(*topology.NodeType.Connector)
			}

			if topology.NodeType.Worker != nil {
				model.NodeTypeWorker = types.BoolValue(*topology.NodeType.Worker)
			}
		}
	...
	return types.ListValueFrom(ctx, types.ObjectType{AttrTypes: enterpriseSearchTopologyAttrTypes()}, result)
}
